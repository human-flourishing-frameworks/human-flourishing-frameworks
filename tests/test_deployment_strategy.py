...
CANONICAL_START = "gunicorn safe_app:app --bind 0.0.0.0:$PORT --log-file -"


class DeploymentStrategyTests(unittest.TestCase):
    def test_railway_config_uses_safe_app_gunicorn_and_healthz(self):
        data = json.loads((ROOT / "railway.json").read_text(encoding="utf-8"))
        self.assertEqual(data["build"]["builder"], "NIXPACKS")
        self.assertEqual(data["deploy"]["startCommand"], CANONICAL_START)
    ...
    def test_render_config_matches_canonical_start(self):
        ...
        self.assertIn(f"startCommand: {CANONICAL_START}", text)
    ...
    def test_deployment_strategy_doc_blocks_stale_commands(self):
        ...
        self.assertIn(CANONICAL_START, text)
